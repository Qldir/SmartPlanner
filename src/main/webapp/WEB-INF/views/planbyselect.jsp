...
<body>
	...
	<script>
		...
		$(function() {
			...
			$("#hotelAddress").html("宿所位置 : "+${result}.plan_startlocation); 
			
			/* 일정 날짜 ======================================================================*/
			var yearFrom = ${result}.plan_from.split(" ")[0].split("/")[2];
			var monthFrom = ${result}.plan_from.split(" ")[0].split("/")[0];
			var dayFrom = ${result}.plan_from.split(" ")[0].split("/")[1];
			var dateFrom = new Date(yearFrom,monthFrom,dayFrom);
			
			var yearTo = ${result}.plan_to.split(" ")[0].split("/")[2];
			var monthTo = ${result}.plan_to.split(" ")[0].split("/")[0];
			var dayTo = ${result}.plan_to.split(" ")[0].split("/")[1];
			...
			for(var i=0;i<dateN;i++){
				...
				if(i==0){
					...
					planday += "<form id=planday_"+i+" class='plan-day'>";
			...
			$("#insertPlanButton").on('click',function(){
				var startDate = new Date(${result}.plan_from.split(" ")[0]);
				var dayPlanList=[];
				for(var i=0;i<dateN;i++){
			...
			$("#routeSearchButton").on('click', function(){
				var placeList=[]
				placeList.push(${result}.plan_startlocation);
				...
				$.ajax({
					...
					success:function(data){
						...
						setWayPointsDirectionsMinRoute(resultPlaceList, ${result}.plan_startlocation);
		...
		function initMap() {
			...
			geocoder.geocode({
				'address' : ${result}.plan_startlocation
